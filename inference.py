...
from ProteinFunctions.src.models.ProtNoteTrainer import ProtNoteTrainer
from ProteinFunctions.src.models.ProtNote import ProtNote
...
parser.add_argument(
    "--window-size", type=int, help="The size of moving window for mutations"
)
...
OUTPUT_PATH = (
    "data/embeddings/inference_E5_multiling_inst_frozen_label_embeddings_mean.pt"
)
...
descriptions_file = pd.DataFrame(
    {"name": names, "label": labels, "synonym_exact": synonym_exact}, index=labels_idxs
)
...
label_encoder = AutoModel.from_pretrained("intfloat/multilingual-e5-large-instruct").to(
    DEVICE
)
...
for go_term, desriptions in tqdm(
    descriptions_file[["name", "label", "synonym_exact"]].iterrows(),
    total=len(descriptions_file),
):
...
for window_start in range(len(sequence) - args.window_size + 1):
    ...
    sampled_sequence = BM.corrupt_sequence_at_locations(
        sequence, locations, "non-conservative", sample=False
    )
    id = f"{window_start}-{window_end}:{sequence[window_start:window_end+1]}->{sampled_sequence[window_start:window_end+1]}"
    records.append(
        SeqRecord(Seq(sampled_sequence), id=id, description=" ".join(labels_idxs))
    )
...
loss_fn = get_loss(config=config, bce_pos_weight=1, label_weights=None)
...
test_metrics = Trainer.evaluate(
    ...
    eval_metrics=eval_metrics.get_metric_collection_with_regex(
        pattern="f1_m.*", threshold=0.3, num_labels=label_sample_sizes["test"]
    ),
    save_results=True,
