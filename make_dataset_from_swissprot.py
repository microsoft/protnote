...
def process_data(
    ...
    label_ids_2019 = set(
        pd.read_pickle("data/annotations/go_annotations_2019_07_01.pkl").index
    )
    ...
    for parent, children in reverse_parenthood.items():
        ...
        if (
            "GO" in parent
            and len(children) == 1
            and leaf_node in annotations_2024.index
        ):
    ...
    if sequence_vocabulary == "new":
        ...
        in_proteinfer_train_val = df_2024.seq_id.apply(lambda x: x in sequence_ids_2019)
    ...
    if label_vocabulary == "proteinfer":
        vocab = set(
            generate_vocabularies(
                "data/swissprot/proteinfer_splits/random/full_GO.fasta"
            )["label_vocab"]
        )
    ...
    df_2024["go_ids"] = df_2024.go_ids.apply(lambda x: (set(x) & vocab))
    ...
    records = [
        ...
        for _, row in SwissProt_2023.iterrows()
    ]
...
if __name__ == "__main__":
    ...
    # updated test set: python make_dataset_from_swissprot.py --data-file-path data/swissprot/uniprot_sprot.dat --output-file-path data/swissprot/test_jul_2024.fasta --only-unseen-seqs --label-vocabulary=new --parenthood-file-path data/vocabularies/parenthood_jul_2024.json
    """
    parser = argparse.ArgumentParser(
        description="Process SwissProt data and generate a FASTA file."
    )
    parser.add_argument(
        "--data-file-path", type=str, help="Path to the SwissProt data file."
    )
    parser.add_argument(
        "--output-file-path",
        type=str,
