...
class ProteinferTest(parameterized.TestCase):
    def test_gcs_path_to_relative_unzipped_path(self):
        ...
        expected = (
            "noxpd2_cnn_swissprot_go_random_swiss-cnn_for_swissprot_go_random-13703706"
        )
    ...
    def test_format_output_adds_description_and_formats_float_confidence(self):
        ...
        actual = proteinfer.format_df_for_output(
            input_df, label_to_description, num_decimal_places
        )
        expected = pd.DataFrame(
            {
                "sequence_name": ["SEQ_A"],
    ...
    @parameterized.parameters(
        dict(input_confidence=1.0, num_decimal_places=2, expected=1.0),
        dict(input_confidence=1.0, num_decimal_places=3, expected=1.0),
    ...
    def test_format_float_confidence(
        self, input_confidence, num_decimal_places, expected
    ):
    ...
    def test_load_models_raises_on_model_missing_no_ensemble(self):
        expected_err_contents = (
            "Unable to find cached models in FAKE_PATH. Make "
