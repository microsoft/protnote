...
DATASET_FEATURES = {
    ...
    LABEL_KEY: tf.FixedLenSequenceFeature(
        [],
        dtype=tf.string,
...
def _is_sequence_short_enough_for_training(example):
    return tf.greater(tf.constant(MAX_SEQUENCE_LENGTH), example[SEQUENCE_LENGTH_KEY])


def non_batched_dataset(train_dev_or_test, label_vocab, dataset_files):
    ...
    if train_dev_or_test not in DATA_FOLD_VALUES:
        raise ValueError(
            (
                "Only train, dev, test and * are supported datasets." " Received {}."
            ).format(train_dev_or_test)
        )

    tfrecord_dataset = tf.data.TFRecordDataset(dataset_files)

    dataset = tfrecord_dataset.map(
        lambda record: tf.io.parse_single_example(  # pylint: disable=g-long-lambda
    ...
    amino_acid_table = contrib_lookup.index_table_from_tensor(
        utils.AMINO_ACID_VOCABULARY, default_value=len(utils.AMINO_ACID_VOCABULARY)
    )
    protein_class_table = contrib_lookup.index_table_from_tensor(mapping=label_vocab)
...
def batched_dataset(input_dataset, batch_size, bucket_boundaries):
    """Batches and pads input_dataset.

    Args:
      input_dataset: tf.data.Dataset. output of _non_padded_dataset.
      batch_size: int.
      bucket_boundaries: sequence lengths at boundaries of buckets.
    ...
    Returns:
      ...
      features are padded with 0 and -1, respectively. The batch_size varies with
      sequenence length, bucketed by bucket_boundaries.
    """

    def _get_element_length(features):
        length = tf.shape(features[SEQUENCE_KEY])[0]
        return length
...
def make_input_fn(batch_size, data_file_pattern, train_dev_or_test, label_vocab):
    ...
    def _input_fn():
        ...
        dataset = batched_dataset(
            dataset, batch_size, bucket_boundaries=BUCKET_BOUNDARIES
        )
...
def yield_examples(tfrecord_path):
    """Reads TfRecords of protein TfExamples and yields examples as dicts.

    Args:
      tfrecord_path: path to TfRecord files of TfExamples.

    Yields:
    example dict with keys of SEQUENCE_ID_KEY, SEQUENCE_KEY and LABEL_KEY,
      containing those values (string, string, list of strings).
    """
    dataset = tf.data.TFRecordDataset(tf.gfile.Glob(tfrecord_path))
    oneshot = dataset.make_one_shot_iterator()
    iterator_get_record = oneshot.get_next()
    with tf.Session() as sess:
        while True:
            try:
                example_string = sess.run(iterator_get_record)
