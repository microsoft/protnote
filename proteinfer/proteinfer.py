...
def _gcs_path_to_relative_unzipped_path(p):
    ...
    return os.path.join(os.path.basename(os.path.normpath(p)).replace(".tar.gz", ""))


def _get_inferrer_paths(model_urls, model_cache_path):
...
def input_text_to_df(input_text):
    ...
    with io.StringIO(initial_value=input_text) as f:
        ...
        fasta_df = pd.DataFrame(
            [(f.name, str(f.seq)) for f in fasta_records],
...
def perform_inference(input_df, models, reporting_threshold):
    ...
    """
    predictions = []
    for inferrer_list in tqdm.tqdm(models, position=1, desc="Progress", leave=True):
        predictions.append(
            inference.get_preds_at_or_above_threshold(
                input_df, inferrer_list, reporting_threshold
            )
        )
...
def _format_float_confidence_for_output(input_float, num_decimal_places):
    # Create a separate function so as to test it against our expectations.
    return np.around(input_float, num_decimal_places)


def format_df_for_output(predictions_df, label_to_description, num_decimal_places):
...
def run(input_text, models, reporting_threshold, label_to_description):
    ...
    predictions_df = perform_inference(
        input_df=input_df, models=models, reporting_threshold=reporting_threshold
    )
...
def load_assets_and_run(
    ...
    label_to_description = utils.load_gz_json(
        os.path.join(model_cache_path, utils.INSTALLED_LABEL_DESCRIPTION_FILE_NAME)
    )

    predictions_df = run(input_text, models, reporting_threshold, label_to_description)
...
def main(_):
    ...
    if FLAGS.reporting_threshold == 0.0:
        raise ValueError(
