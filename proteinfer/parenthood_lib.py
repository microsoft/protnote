...
class GoTerm(
    ...
    def from_string(cls, s):
        ...
        """
        lines = s.split("\n")
        attributes = [_parse_go_attribute(l) for l in lines if _is_go_attribute_line(l)]
        term_ids = [value for tag, value in attributes if tag == "id"]
        if len(term_ids) != 1:
            raise ValueError(
                (
                    "Number of term names for term was {} "
                    "(expected exactly one). Term was {}."
                ).format(len(term_ids), attributes)
            )
        term_id = term_ids[0]
        ...
        for tag, value in attributes:
            ...
            if (
                tag in _PARENTHOOD_TYPE_GO_RELATIONS
                or tag in _IDENTITY_TYPE_GO_RELATIONS
            ):
                ...
                valid_relations = _PARENTHOOD_TYPE_GO_RELATIONS.union(
                    _NON_PARENTHOOD_TYPE_GO_RELATIONS
                ).union(_IDENTITY_TYPE_GO_RELATIONS)
                raise ValueError(
                    "Term type unknown: was {} and expected one of {}. "
...
def _get_go_term_from_text(s):
    ...
    if len(matches) != 1:
        raise ValueError(
            "Expected exactly one match for a GO term in string {}. Found matches {}".format(
                s, matches
            )
        )
...
def _yield_terms_for_alt_ids(term):
    ...
    for relation, related_term_id in term.related_labels:
        if relation == "alt_id":
            ...
            non_canonical_term = GoTerm(
                related_term_id, term.term_name, term.description, related_labels
            )
...
def parse_full_go_file(file_contents=None):
    ...
    unparsed_terms = [x for x in file_contents.split("\n\n") if x.startswith("[Term]")]
    parsed_terms = [GoTerm.from_string(t) for t in unparsed_terms]
    with_alt_ids_itr = itertools.chain(
        *(_yield_terms_for_alt_ids(x) for x in parsed_terms)
    )

    return list(with_alt_ids_itr)


def go_label_to_description(go_file_contents=None):
    return {t.term_id: f"{t.term_name}" for t in parse_full_go_file(go_file_contents)}
...
def _go_term_applicable_labels_should_include_themselves(term):
    ...
    """
    is_replaced_by = any(
        relation_type == "replaced_by" for relation_type, _ in term.related_labels
    )
    alt_id = any(
        relation_type == _IS_NON_CANONICAL_ALT_ID_LABEL_OF
...
def transitive_go_parenthood(go_terms):
    ...
    """
    go_term_dict = {
        t.term_id: frozenset(label for _, label in t.related_labels) for t in go_terms
    }
...
def _transitive_parenthood(key, term_dict):
...
def _replace_one_level_up_with_dash_for_ec(s):
    ...
    raise ValueError(
        "Expected the number of hyphens in string to be between "
...
def _get_leaf_node_ec_labels_from_file_contents(enzyme_dat_file_contents=None):
...
def _get_non_leaf_node_ec_labels_from_file_contents(enzyme_class_file_contents=None):
...
def ec_label_to_description(
    enzyme_dat_file_contents=None, enzyme_class_file_contents=None
):
...
def get_applicable_label_dict(path=APPLICABLE_LABEL_JSON_PATH):
...
def is_implied_by_something_else(
    ...
    children_of_current_label = children_of_current_label - frozenset([current_label])

    return (
        len(  # pylint: disable=g-explicit-length-test
            children_of_current_label.intersection(
                all_labels_for_protein_without_current
            )
        )
        > 0
    )
...
def _filter_label_set_to_most_specific(label_set, reversed_normalizer):
