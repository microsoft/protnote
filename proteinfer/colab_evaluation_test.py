...
class ColabEvaluationTest(parameterized.TestCase):
    ...
    def test_batched_inferences_from_dir(self, batch_size, num_examples=100):
        ...
        (
            ...
            accessions_list,
            activations_list,
        ) = self._generate_random_inferences(num_examples)
        ...
        if batch_size > 1:
            np.testing.assert_equal(actual[1][1][1], activations_list[batch_size + 1])
    ...
    def test_merge_predictions_and_ground_truth(self):
        ...
        actual_df = colab_evaluation.merge_predictions_and_ground_truth(pred, gt)
        expected_df = pd.DataFrame(
            {
                "up_id": ["SEQ0", "SEQ0", "SEQ1", "SEQ1", "SEQ1", "SEQ3"],
                "label": ["ENTRY1", "ENTRY2", "ENTRY0", "ENTRY1", "ENTRY2", "ENTRY6"],
    ...
    def test_get_pr_curve_df(self):
        ...
        np.testing.assert_almost_equal(
            pr_curve["recall"], np.array([1, 0.75, 0.75, 0.5])
        )
        np.testing.assert_almost_equal(
    ...
    def test_apply_threshold_and_return_stats(self):
        ...
        actual = colab_evaluation.apply_threshold_and_return_stats(
            pred,
            gt,
            grouping={"ENTRY0": "A", "ENTRY1": "A", "ENTRY2": "A", "ENTRY6": "A"},
        )
        expected = pd.DataFrame(
            {
                "group": ["A"],
