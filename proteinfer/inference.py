...
def in_graph_inferrer(sequences, savedmodel_dir_path, signature, name_scope="inferrer"):
...
def memoized_inferrer(
    savedmodel_dir_path,
    activation_type=tf.saved_model.signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY,
    batch_size=16,
    use_tqdm=False,
    session_config=None,
...
class Inferrer(object):
    ...
    def __repr__(self):
        return (
            "{} with feed tensors savedmodel_dir_path {} and " "activation_type {}"
        ).format(type(self).__name__, self._savedmodel_dir_path, self.activation_type)

    def _get_tensor_by_name(self, name):
        return self._graph.get_tensor_by_name(
            "{}/{}".format(self._model_name_scope, name)
        )

    def _get_activations_for_batch_unmemoized(
        self, seqs, custom_tensor_to_retrieve=None
    ):
    ...
    @functools.lru_cache(maxsize=None)
    def _get_activations_for_batch_memoized(self, seqs, custom_tensor_to_retrieve=None):
    ...
    def get_activations(self, list_of_seqs, custom_tensor_to_retrieve=None):
        ...
        if len(np_seqs.shape) != 1:
            raise ValueError(
                "`list_of_seqs` should be convertible to a numpy vector "
        ...
        batches = np.array_split(
            np_seqs[sorter], np.ceil(len(np_seqs) / self.batch_size)
        )
        if self._use_tqdm:
            batches = tqdm.tqdm(
                batches,
        ...
        for batch in batches:
            ...
            batch_activations_sparse = [
                scipy.sparse.coo_matrix(x) for x in batch_activations
            ]
...
def latest_savedmodel_path_from_base_path(base_path):
    ...
    suffixes = [
        x for x in tf.io.gfile.listdir(protein_export_base_path) if "temp-" not in x
    ]
...
def predictions_for_df(df, inferrer):
    ...
    working_df["predictions"] = inferrer.get_activations(
        working_df.sequence.values
    ).tolist()
    return working_df


def serialize_inference_result(sequence_name, activations):
...
def deserialize_inference_result(results_b64):
    ...
    if len(single_pred_dict) != 1:
        raise ValueError(
            "Expected exactly one object in the structured np array. "
...
def get_preds_at_or_above_threshold(input_df, inferrer_list, threshold):
    ...
    if threshold == 0.0:
        raise ValueError(
            "The given threshold was 0. Please supply a "
    ...
    for idx, protein_sparse in enumerate(predictions):
        ...
        for label, confidence in zip(
            labels_predicted, protein[proteins_above_threshold]
        ):
