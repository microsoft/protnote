...
def stats_by_group(df):
    ...
    result["proportion_text"] = (result["proportion"] * 100).round(2).astype(str) + "%"
    return result
...
def apply_threshold_and_return_stats(
    predictions_df, ground_truth_df, threshold=0.5, grouping=None
):
    ...
    if grouping:
        ...
        return stats_by_group(calls.groupby("group")).assign(threshold=threshold)
    else:
...
def _make_tidy_df_from_seq_names_and_prediction_array(
    sequence_names, predictions_array, vocab, min_decision_threshold=1e-20
):
...
def get_normalized_inference_results(
    shard_dir_path, vocab, label_normalizer, min_decision_threshold=1e-20
):
...
def filter_pr_curve(precisions, recalls, thresholds, resolution=1e-4):
    ...
    for i in range(len(precisions)):
        if last_precision is None or abs(recalls[i] - last_recall) >= resolution:
...
def assign_tp_fp_fn(predictions_df, ground_truth_df, threshold):
    ...
    combined["fp"] = (combined["gt"] == False) & (combined["value"] > threshold)
