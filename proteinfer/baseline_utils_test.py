...
class BaselineUtilsTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        dict(
            ...
            input_row=pd.Series({"predicted_label": frozenset([]), "bit_score": 99.0}),
        ...
        dict(
            ...
            input_row=pd.Series(
                {"predicted_label": frozenset(["PF00001"]), "bit_score": 99.0}
            ),
            input_label_vocab=np.array(["PF00001"]),
            expected=[99.0],
        ),
    ...
    def test_blast_row_to_confidence_array(
        self, input_row, input_label_vocab, expected
    ):
    ...
    def test_load_blast_output(self):
        ...
        input_blast = 'accession="ACCESSION"\taccession="MATCHACCESSION"\t82.456\t57\t10\t0\t1\t57\t1\t57\t6.92e-21\t79.3\n'
        ...
        expected = pd.DataFrame(
            {
                ...
                "true_label": [{"GO:101010", "EC:9.9.9.9"}, {"EC:1.2.-.-"}],
                "predicted_label": [
                    {"GO:101010", "EC:9.9.9.9", "Pfam:PF12345"},
                    frozenset(),
                ],
    ...
    def test_limit_set_of_labels(self):
        # Set up input data.
        input_df = pd.DataFrame({"labels": [frozenset(["a"]), frozenset(["a", "b"])]})
        ...
        # Compute actual.
        actual = baseline_utils.limit_set_of_labels(
            input_df, acceptable_labels, column_to_limit
        )
...
NOT_USED""",
    ...
    def test_load_interproscan_output(
        self,
        interproscan_output,
