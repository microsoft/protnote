...
class TestEvaluation(parameterized.TestCase):
    ...
    def test_normalize_confidences_edge(self, confidences, vocab, normalized, parents):
        test_normalized = evaluation.normalize_confidences(confidences, vocab, parents)
        np.testing.assert_array_equal(test_normalized, normalized)

    def test_normalize_confidences(self):
        test_confidences = np.array(
            [[0.2, 0.5, 0.1, 0.2], [0.1, 0.5, 0.2, 0.1], [0.1, 0.1, 0.6, 0.2]]
        )
        test_vocab = ["a", "b", "c", "d"]
        test_parents = {"b": ["a", "b", "c"], "d": ["d"], "a": ["a"], "c": ["c"]}

        normalized = evaluation.normalize_confidences(
            test_confidences, test_vocab, test_parents
        )
    ...
    @parameterized.named_parameters(
        dict(
            ...
            expected_precision=np.array([4.0 / 5, 1.0]),
            # Threshold of .4 gives 3 found out of 4.
            expected_recall=np.array([1.0, 3.0 / 4]),
        ),
        dict(
    ...
    def test_get_pr_f1_df(self, expected_precision, expected_recall, eval_subset=None):
        df = pd.DataFrame(
            dict(
                true_label=[{"a"}, {"a", "b", "c"}],
        ...
        np.testing.assert_array_equal(pr_f1_df.threshold.values, expected_thresholds)
    ...
    def test_get_pr_f1_df_precision_truncation(self):
        ...
        self.assertLess(
            len(pr_f1_df_limited_precision),
    ...
    def test_multilabel_f1_per_example_label_pair(self):
        ...
        expected = 0.8  # Harmonic mean of 1. and 2./3
    ...
    def test_precision_recall_f1_integration_test(self):
        ...
        expected_precision = 1.0  # CLASS_0 is normalized out.
        expected_recall = 1.0  # Both sequences have their true label recalled.
        expected_f1 = 1.0
    ...
    @parameterized.named_parameters(
        ...
        dict(
            ...
            input_predictions=np.array([[0.5, 0.5], [0.5, 0.5]]),
            decision_threshold=1.0,
        ...
        dict(
            ...
            input_predictions=np.array([[1.0, 0.5], [0.5, 0.5]]),
        ...
        dict(
            ...
            input_predictions=np.array([[1.0, 0.5], [0.5, 1.0]]),
            decision_threshold=0.75,
            label_vocab=np.array(["class1", "class2"]),
            expected=[frozenset(["class1"]), frozenset(["class2"])],
        ),
        dict(
            testcase_name="two sequences, all classes are predicted",
            input_predictions=np.array([[1.0, 1.0], [1.0, 1.0]]),
            decision_threshold=0.75,
            label_vocab=np.array(["class1", "class2"]),
            expected=[frozenset(["class1", "class2"]), frozenset(["class1", "class2"])],
        ),
    )
    def test_filter_predictions_to_above_threshold(
        self, input_predictions, decision_threshold, label_vocab, expected
    ):
    ...
    def test_get_predictions_above_threshold(
        self, input_seqs, decision_threshold, vocab_size, expected_count_over_threshold
    ):
        input_df = pd.DataFrame({"sequence_name": input_seqs, "sequence": input_seqs})
        ...
        actual_count_over_threshold = len(
            [s for s in actual.predicted_label if len(s) > 0]
    ...
    def test_pfam_label_normalizer_to_lifted_clan(self, input_normalizer, expected):
    ...
    def test_convert_pfam_ground_truth_to_lifted_clans(self):
        ...
        np.testing.assert_array_equal(
            actual.sequence_name.values, expected.sequence_name.values
        )
        self.assertSetEqual(actual.true_label.values[0], expected.true_label.values[0])
        self.assertSetEqual(actual.true_label.values[1], expected.true_label.values[1])
    ...
    def test_ec_agreement_for_level(
        self, input_level, input_df, expected_agree, expected_disagree, expected_no_call
    ):
        (
            actual_agree,
            actual_disagree,
            actual_no_call,
        ) = evaluation.ec_agreement_for_level(input_df, input_level)

        self.assertEqual(
            actual_agree, expected_agree, "agreement numbers were not the same."
        )
        self.assertEqual(
            actual_disagree,
            expected_disagree,
        ...
        self.assertEqual(
            actual_no_call, expected_no_call, "no-call numbers were not the same."
        )
