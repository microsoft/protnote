...
class InterproScanResult(
    typing.NamedTuple(
        "InterproScanResult", (("sequence_name", Text), ("predicted_label", Set[Text]))
    )
):
    ...
    def from_string(s):
        ...
        if (
            len(tab_split) < 14 or tab_split[13] == ""
...
def load_blast_output(
    filename, label_vocab, training_data_ground_truth, test_data_ground_truth
):
    ...
    training_data_ground_truth_lookup = dict(
        list(
            zip(
                training_data_ground_truth.sequence_name,
    ...
    with tf.io.gfile.GFile(filename) as f:
        for line in tqdm.tqdm(f, position=0):
            blast_output_namedtuples.append(
                BlastResult.from_string(line, training_data_ground_truth_lookup)
            )
    ...
    blast_output["bit_score"].fillna(0.0, inplace=True)
...
def limit_set_of_labels(df, acceptable_labels, column_to_limit):
...
def limit_labels_for_label_normalizer(label_normalizer, acceptable_labels):
    ...
    for k, v in label_normalizer.items():
        if k in acceptable_labels:
            limited_label_normalizer[k] = sorted(acceptable_labels.intersection(set(v)))
...
def load_interproscan_output(
    ...
    with tf.io.gfile.GFile(interproscan_output_filename) as f:
        for line in tqdm.tqdm(f, position=0):
            interproscan_output_namedtuples.append(InterproScanResult.from_string(line))

    interproscan_output = pd.DataFrame.from_records(
        interproscan_output_namedtuples, columns=InterproScanResult._fields
    )

    collapsed_interproscan_output_dict = {"sequence_name": [], "predicted_label": []}
    ...
    collapsed_interproscan_output = pd.DataFrame(collapsed_interproscan_output_dict)
