...
class _InferrerFixture(object):
    ...
    def get_activations(self, input_seqs):
        ...
        """
        dense = np.reshape(
            [len(s) for s in input_seqs], [-1] + [1] * (self._activation_rank - 1)
        )
...
class InferenceLibTest(parameterized.TestCase, tf.test.TestCase):
    ...
    def testStringInput(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "`list_of_seqs` should be convertible to a "
    ...
    def test_predictions_for_df(self):
        ...
        self.assertEqual(actual_output_df["predictions"].values.tolist(), [4, 3, 2, 1])
    ...
    def test_serialize_deserialize_inference_result(self):
        ...
        serialized = inference.serialize_inference_result(
            input_accession, input_activations
        )
        (
            deserialized_actual_accession,
            deserialized_actual_activations,
        ) = inference.deserialize_inference_result(serialized)

        self.assertEqual(deserialized_actual_accession, input_accession)
        np.testing.assert_array_equal(
            deserialized_actual_activations, input_activations
        )

    def test_parse_sharded_inference_results(self):
        # Create input inference results.
        input_accession_1 = "ACCESSION_1"
        input_activations_1 = np.array([1.0, 2.0, 3.0])

        input_accession_2 = "ACCESSION_2"
        input_activations_2 = np.array([4.0, 5.0, 6.0])

        input_accession_3 = "ACCESSION_3"
        input_activations_3 = np.array([7.0, 8.0, 9.0])

        # Create files and a directory containing those inference results.
        shard_1_contents = (
            inference.serialize_inference_result(input_accession_1, input_activations_1)
            + b"\n"
    ...
    @parameterized.named_parameters(
        dict(
            ...
            input_df=pd.DataFrame(
                {"sequence_name": ["seq1", "seq2"], "sequence": ["ACDE", "WWWYYY"]}
            ),
            threshold=5.0,
        ...
        dict(
            ...
            input_df=pd.DataFrame(
                {"sequence_name": ["seq2", "seq1"], "sequence": ["WWWYYY", "ACDE"]}
            ),
            threshold=2.0,
            expected=pd.DataFrame(
                {
    ...
    def testGetPredsAboveThresholdRaisesOnZeroThreshold(self):
        ...
        with self.assertRaisesRegex(ValueError, "0"):
            inference.get_preds_at_or_above_threshold(input_df, inferrer_list, 0.0)
