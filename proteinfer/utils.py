...
_OSS_PFAM_ZIPPED_MODELS_URL_BASE = (
    OSS_ZIPPED_MODELS_ROOT_URL
    + "noxpd2_cnn_swissprot_pfam_random_swiss-cnn_for_swissprot_pfam_random-"
)
_OSS_EC_ZIPPED_MODELS_URL_BASE = (
    OSS_ZIPPED_MODELS_ROOT_URL
    + "noxpd2_cnn_swissprot_ec_random_swiss-cnn_for_swissprot_ec_random-"
)
_OSS_GO_ZIPPED_MODELS_URL_BASE = (
    OSS_ZIPPED_MODELS_ROOT_URL
    + "noxpd2_cnn_swissprot_go_random_swiss-cnn_for_swissprot_go_random-"
)
...
def _build_one_hot_encodings():
    ...
    special_mappings = {
        ...
        * (base_encodings[to_aa_index("E")] + base_encodings[to_aa_index("Q")]),
        ...
        _PFAM_GAP_CHARACTER: np.zeros(len(AMINO_ACID_VOCABULARY)),
    }
...
def in_graph_residues_to_onehot(residues):
    ...
    return (
        ragged_one_hots.to_tensor(default_value=0),
        tf.cast(ragged_one_hots.row_lengths(), dtype=tf.int32),
    )


def calculate_bucket_batch_sizes(
    bucket_boundaries, max_expected_sequence_size, largest_batch_size
):
    ...
    bucket_relative_batch_sizes = [
        (first_max_size / x) for x in bucket_boundaries + [max_expected_sequence_size]
    ]
...
def batch_iterable(iterable, batch_size):
    ...
    if batch_size < 1:
        raise ValueError(
            "Cannot have a batch size of less than 1. Received: {}".format(batch_size)
        )
...
def pad_one_hot(one_hot, length):
    if length < one_hot.shape[0]:
        raise ValueError(
            "The padding value must be longer than the one-hot's 0th "
...
def make_padded_np_array(ragged_arrays):
    ...
    return np.array(
        [pad_one_hot(ragged_array, max_array_length) for ragged_array in ragged_arrays]
    )
...
def fetch_oss_pretrained_models(
    model_type, output_dir_path, num_ensemble_elements=None
):
