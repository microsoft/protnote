...
flags.DEFINE_string(
    "label_vocab_path", None, "Relative path (from this file) to csv file of labels."
)

flags.DEFINE_string("output_dir", "/tmp/protein_model", "Path to save checkpoints.")

flags.DEFINE_string(
    "hparams_set",
    hparams_sets.small_test_model.__name__,
    "Hyperparameters to use (see hparams_sets module).",
)

flags.DEFINE_enum(
    "train_fold",
    protein_dataset.TRAIN_FOLD,
    protein_dataset.DATA_FOLD_VALUES,
    "Fold to use for training data " "(one of protein_dataset.DATA_FOLD_VALUES)",
)
...
def _make_estimator(hparams, label_vocab, output_dir):
    ...
    """
    model_fn = protein_model.make_model_fn(label_vocab=label_vocab, hparams=hparams)
...
def _make_estimator_and_inputs(
    ...
    train_spec = tf.estimator.TrainSpec(train_input_fn, max_steps=hparams.train_steps)
...
def parse_label_vocab(label_vocab_path):
    ...
    if set(available_indexes) != set(range(len(available_indexes))):
        raise ValueError(
            "Vocab indexes were not the consecutive integers between "
            "0 (inclusive) and len(vocab) (exclusive). "
            "Got {}.".format(sorted(available_indexes))
        )

    return label_df.sort_values([_VOCAB_INDEX_COLUMN_NAME])[
        _VOCAB_ITEM_COLUMN_NAME
    ].values
