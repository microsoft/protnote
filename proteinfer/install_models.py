...
def download_models(model_cache_path, num_ensemble_elements=None):
    ...
    for model_type in tqdm.tqdm(
        ["Pfam", "EC", "GO"], desc="Overall progress", position=1, leave=True
    ):
        utils.fetch_oss_pretrained_models(
            model_type, model_cache_path, num_ensemble_elements=num_ensemble_elements
        )
...
def get_description_file(model_cache_path):
    out_path = os.path.join(
        model_cache_path, utils.INSTALLED_LABEL_DESCRIPTION_FILE_NAME
    )
...
def run(install_ensemble, model_cache_path):
    ...
    if install_ensemble:
        _logger.warning(
            "Full installation downloads and unpacks ~10GB of data; "
...
def main(_):
    print(FLAGS.model_cache_path)
    run(
        install_ensemble=FLAGS.install_ensemble, model_cache_path=FLAGS.model_cache_path
    )
