...
def normalize_confidences(predictions, label_vocab, applicable_label_dict):
    ...
    children = parenthood_lib.reverse_map(
        applicable_label_dict, set(vocab_indices.keys())
    )
...
def get_ground_truth_multihots(label_sets, label_vocab):
...
def get_pr_f1_df_from_arrays(
    ...
    if prediction_precision_limit:
        normalized_predictions = np.around(
            normalized_predictions, prediction_precision_limit
        )
...
def get_pr_f1_df(
    ...
    normalized_predictions = normalize_confidences(
        prediction_array, label_vocab, label_normalizer
    )
    true_label_sets = prediction_df.true_label.apply(eval_vocab.intersection).values
    eval_indices = np.array([i for i, v in enumerate(label_vocab) if v in eval_vocab])
    ground_truths = get_ground_truth_multihots(
        true_label_sets, label_vocab[eval_indices]
    )
    return get_pr_f1_df_from_arrays(
        ground_truths,
...
def true_false_positive_negative_df(df):
    ...
    """
    dict_prep_for_df = {"sequence_name": [], "class": [], "predicted": [], "true": []}
...
def multilabel_precision_per_example_label_pair(df):
...
def multilabel_recall_per_example_label_pair(df):
...
def multilabel_f1_per_example_label_pair(df):
    """Computes f1 score of predictions in dataframe.
...
def normalize_predictions(to_normalize, normalize_map):
...
def precision_recall_f1(df, label_normalizing_dict):
...
def filter_predictions_to_above_threshold(predictions, decision_threshold, label_vocab):
...
def get_predictions_above_threshold(
    input_df, inferrer, decision_threshold, label_vocab=None
):
...
def _family_and_clan_to_just_clan(family_and_clan):
    ...
    if len(family_and_clan) == 2:
        ...
        raise ValueError(
            "family_and_clan was length 2, but did not have a clan in "
            "it. family_and_clan was {}".format(family_and_clan)
        )

    raise ValueError(
        "Expected either one or two values for family_and_clan. "
        "Was {}".format(family_and_clan)
    )


def pfam_label_normalizer_to_lifted_clan(label_normalizer):
...
def convert_pfam_ground_truth_to_lifted_clans(ground_truth, label_normalizer):
    ...
    """
    lifted_label_normalizer = pfam_label_normalizer_to_lifted_clan(label_normalizer)
...
def ec_agreement_for_level(df, level):
    ...
    if not (level >= 1 and level <= 4):
        raise ValueError("Expected level to be between 1 and 4. Was {}".format(level))
    ...
    disagree = has_ground_truth_label & pred_made & (true_labels != predicted_labels)

    agree_numerator = sum(agree)
    disagree_numerator = sum(disagree)
    no_pred_made_numerator = sum(~pred_made & has_ground_truth_label)

    if agree_numerator + disagree_numerator + no_pred_made_numerator != sum(
        has_ground_truth_label
    ):
        # Internal correctness check, so raise an AssertionError, not a ValueError.
        error_msg = (
            f"Expected the sum agree_numerator + disagree_numerator + "
