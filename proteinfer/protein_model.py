...
def _f1_score(labels, predictions):
    ...
    return (
        2 * precision[0] * recall[0] / (precision[0] + recall[0] + 1e-5),
...
def _custom_recall_at_k(labels_as_multi_hot, predictions, k):
    ...
    predictions_top_k_as_multi_hot = _indices_to_multihot(indices, num_output_classes)
    ...
    false_negatives_tensor = tf.math.greater(
        labels_as_multi_hot, predictions_top_k_as_multi_hot
    )
    ...
    recall_per_example_finite_only = tf.boolean_mask(recall_per_example, is_finite)
...
def _make_evaluation_metrics(labels, predictions, num_output_classes, hparams):
    ...
    predictions_as_bools = tf.greater(
        predictions_as_floats, tf.constant(recall_threshold)
    )

    metrics = {
        "precision_at_threshold": tf.metrics.precision(
            labels=multi_hot_labels, predictions=predictions_as_bools
        ),
        "recall_at_threshold": tf.metrics.recall(
            labels=multi_hot_labels, predictions=predictions_as_bools
        ),
        "f1_at_threshold": _f1_score(
            labels=multi_hot_labels, predictions=predictions_as_bools
        ),
        "mean_examplewise_f1_at_threshold": _mean_examplewise_f1_score(
            labels=multi_hot_labels, predictions=predictions_as_bools
        ),
    ...
    for k in _THRESHOLDS_FOR_RECALL_METRIC:
        metrics["recall@%d" % k] = _custom_recall_at_k(
            labels_as_multi_hot=multi_hot_labels, predictions=predictions_as_floats, k=k
        )

    return metrics


def _set_padding_to_sentinel(padded_representations, sequence_lengths, sentinel):
    ...
    with tf.variable_scope("set_padding_to_sentinel", reuse=False):
        longest_sequence_length = tf.shape(padded_representations)[sequence_dimension]
...
def _make_per_sequence_features(per_location_representations, raw_features, hparams):
    ...
    per_location_representations = _set_padding_to_sentinel(
        per_location_representations, sequence_lengths, tf.constant(0.0)
    )
    ...
    denominator = (
        tf.cast(
            tf.expand_dims(raw_features[protein_dataset.SEQUENCE_LENGTH_KEY], axis=-1),
            tf.float32,
        )
        ** hparams.denominator_power
    )

    pooled_representation = (
        tf.reduce_sum(per_location_representations, axis=1) / denominator
    )
...
def _convert_representation_to_prediction_ops(
    representation, raw_features, num_output_classes, hparams
):
    ...
    logits = tf.layers.dense(per_sequence_features, num_output_classes, name=LOGITS_KEY)

    predictions = {
        protein_dataset.LABEL_KEY: tf.identity(tf.sigmoid(logits), name="predictions")
    }
...
def _conv_layer(
    sequence_features, sequence_lengths, num_units, dilation_rate, kernel_size
):
    """Return a convolution of the input features that respects sequence len."""
    padding_zeroed = _set_padding_to_sentinel(
        sequence_features, sequence_lengths, tf.constant(0.0)
    )
    ...
    re_zeroed = _set_padding_to_sentinel(conved, sequence_lengths, tf.constant(0.0))
    return re_zeroed
...
def _make_loss(predictions_for_loss, labels, num_output_classes):
    ...
    loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=labels_op, logits=logits)
...
def _make_train_op(loss, hparams):
    ...
    def learning_rate_decay_fn(learning_rate, global_step):
        learning_rate = tf.train.exponential_decay(
            learning_rate, global_step, hparams.lr_decay_steps, hparams.lr_decay_rate
        )
        learning_rate = learning_rate * tf.minimum(
            tf.cast(global_step / hparams.lr_warmup_steps, tf.float32), tf.constant(1.0)
        )
