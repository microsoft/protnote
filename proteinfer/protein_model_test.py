...
class ProteinModelTest(parameterized.TestCase):
    def testF1Score(self):
        ...
        # Specificity: 1/2
        # F1-score: 1/2
        predictions = tf.constant([[1, 0], [0, 1], [1, 0], [0, 1]], dtype=tf.float32)
        f1, update_op = protein_model._f1_score(labels, predictions)
        with tf.Session() as sess:
            sess.run(tf.local_variables_initializer())
            sess.run(update_op)
            self.assertAlmostEqual(1.0 / 2, f1.eval(), places=4)

    def testMeanExampleWiseF1Score(self):
        """Tests the F1 score metric."""
        labels = tf.constant([[1, 0], [0, 1], [0, 1], [1, 0], [0, 0]], dtype=tf.int32)
        # Sensitivity: 1/2
        # Specificity: 1/2
        # F1-score: 1/2
        predictions = tf.constant(
            [[1, 0], [0, 1], [1, 0], [0, 1], [1, 1]], dtype=tf.float32
        )
        f1, update_op = protein_model._mean_examplewise_f1_score(labels, predictions)
        with tf.Session() as sess:
            sess.run(tf.local_variables_initializer())
            sess.run(update_op)
            self.assertAlmostEqual(1.0 / 2, f1.eval(), places=4)
    ...
    def testRecallAtK(self):
        labels = tf.convert_to_tensor(
            [[1, 0, 0], [0, 1, 1], [0, 0, 0]], dtype=tf.float32
        )
        predictions = tf.convert_to_tensor(
            [[0.5, 1.0, 0.0], [0.5, 0.0, 1.0], [0.25, 0.25, 0.4]]
        )
    ...
    @parameterized.named_parameters(
        dict(
            ...
            padded_representations=[[[11.0], [21.0], [31.0]], [[41.0], [51.0], [61.0]]],
        ...
        dict(
            ...
            expected=[[[11.0], [21.0], [31.0]], [[41.0], [51.0], [61.0]]],
            sentinel=0.0,
        ),
        dict(
        ...
        dict(
            ...
            padded_representations=[[[11.0], [21.0], [31.0]], [[41.0], [51.0], [61.0]]],
            sequence_lengths=[0, 0],
            expected=[[[-99.0], [-99.0], [-99.0]], [[-99.0], [-99.0], [-99.0]]],
            sentinel=-99.0,
        ),
        dict(
            testcase_name="embedding dimension size > 1",
            padded_representations=[
                [[11.0, -1.0], [21.0, -2.0], [31.0, -3.0]],
                [[41.0, -4.0], [51.0, -5.0], [61.0, -6.0]],
            ],
            sequence_lengths=[2, 3],
            expected=[
                [[11.0, -1.0], [21.0, -2.0], [0.0, 0.0]],
                [[41.0, -4.0], [51.0, -5.0], [61.0, -6.0]],
            ],
            sentinel=0.0,
        ),
    )
    def testSetPaddingToSentinel(
        self, padded_representations, sequence_lengths, expected, sentinel
    ):
        with tf.Graph().as_default():
            with tf.Session() as sess:
                ...
                actual = sess.run(
                    protein_model._set_padding_to_sentinel(
                        padded_representations, sequence_lengths, sentinel
                    )
                )
