...
def main(
    ...
    proteinfer_predictions_path = (
        f"outputs/results/test_logits_{annotation_type}_{test_name}_proteinfer.parquet"
    )
    ...
    if (
        not os.path.exists(
            f"outputs/results/test_1_labels_{test_name}_{model_name}.parquet"
        )
    ) | (not cache):
        logger.info("Running zero_shot model...")
        subprocess.run(
            TEST_COMMANDS[test_name]
            .replace(MODEL_PATH_TOKEN, f"models/ProtNote/{model_name}.pt")
    ...
    zero_shot_labels = pd.read_parquet(
        f"outputs/results/test_1_labels_{test_name}_{model_name}.parquet"
    )
    vocabularies = generate_vocabularies(
        file_path=f"data/swissprot/proteinfer_splits/random/full_GO.fasta"
    )
    ...
    if annotation_type == "EC":
        ...
        embeddings_idx_new = torch.load(
            f"data/embeddings/{label_embeddings_new}_index.pt"
        )
    ...
    zero_shot_pinf_baseline_logits.to_parquet(
        f"outputs/results/test_logits_{annotation_type}_{test_name}_{label_embedding_model}_baseline.parquet"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The name of the latest zero shot model without the .pt",
    )
    parser.add_argument(
